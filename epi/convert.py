...
def converter(model, dati, regione, Nc):
    ...
    if model == 'SUIHTER':
        ...
        res_np[:, 3] = res['ricoverati_con_sintomi'].values + \
                       res['terapia_intensiva'].values + \
                       res['isolamento_domiciliare'].values
        res_np[:, 4] = res['dimessi_guariti'].values
        res_np[:, 5] = res['deceduti'].values
    if model == 'SUIHTER':
        ...
        results_df =pd.DataFrame(res_np, columns=['Geocode', 'time', 'Suscept', 'Infected', 'Recovered', 'Deceased', 'VaccinatedFirst', 'VaccinatedSecond'])
        results_df['data'] = res['data']
