...
def model_fun_var_new(q,data):
    ...
    Y0[6] *= q[-1]
    Y0[0] = args[3] - Y0[1:].sum()
    args[9][0] = data.ydata[0][-2,0]
    res = es.solve_rk4(getattr(md, epi_model + 'model'), [t_data[0], t_data[-1]], Y0, t_data[1] - t_data[0], args=args[2:-2])
    last = len(data.ydata[0][0])-1
    Y0 = np.zeros(res.shape[0]+1)
    #res[1,last] += 500/args[2].delta(last)
    Y0[1] = res[1,last] * (1 - variant_perc) 
    Y0[2] = res[1,last] * variant_perc
    Y0[7:] = res[6:,last]
    ...
    Y0[0] = res[0, last] 
    t_data = t_data[last:]
    args[9][last] = data.ydata[0][-2,last]
    res = es.solve_rk4(getattr(md, epi_model + 'model_variant'), [t_data[0], t_data[-1]], Y0, t_data[1] - t_data[0], args=args[2:-1]+[variant_perc, variant_factor, kappa1, kappa2])
...
def model_ss(params,data):
    ...
    RtotD = es.postProcessH(args[2], t_data, res[2:3], res[3:4], res[4:5], args[5]).flatten('F') + ydata[-2,0]
    ...
    if res.shape[0]==5:
        ...
        #ss[4] = (((RtotD - ydata[-2])**2)/np.maximum(ydata[-2],np.ones(ydata[-2].shape))).sum()
        ss[4] = (((RtotD - ydata[-2])**2)*0.1/max(ydata[-2])).sum()
        ss[5] = (((new_pos - ydata[-1])**2)*0.1 / np.maximum(ydata[-1],np.ones(ydata[-1].shape))).sum()
    return ss
