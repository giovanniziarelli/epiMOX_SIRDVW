...
if __name__=='__main__':
    ...
    model_solver.save()
    # Running the simulation for other six weeks 
    weeks = 6
    model_solver.US1 = np.append(model_solver.US1, np.zeros((model_solver.Ns, weeks)), axis = 1)
    model_solver.UR = np.append(model_solver.UR, np.zeros((model_solver.Ns, weeks)), axis = 1)
    model_solver.US2 = np.append(model_solver.US2, model_solver.US1[:, -(weeks+3):-weeks], axis = 1)
    model_solver.US2 = np.append(model_solver.US2, np.zeros((model_solver.Ns,weeks-3)), axis = 1)
    model_solver.DPC_end += datetime.timedelta(weeks*7)
    model_solver.t_list = np.arange(0,(model_solver.DPC_end - model_solver.DPC_start).days+1)
    model_solver.Y = np.append(model_solver.Y, np.zeros((model_solver.Nc, weeks*7, model_solver.Ns)), axis =1) 
    model_solver.Sfrac = np.append(model_solver.Sfrac, np.zeros((weeks*7, model_solver.Ns)), axis = 0)
    model_solver.save_weeks_plus(weeks)
